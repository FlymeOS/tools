...
def show(key, category=None):
    ...
    if item.detail is not None:
        print "%s(%d): %s" % (Paint.bold(item.name), item.code, item.detail.replace("\t", "").strip())
...
def readme():
    ...
    print >>f, "                                                                 "
    print >>f, "***An open source project for Android ROM porting***"
    print >>f, "                                                                 "
    print >>f, "Apache License 2.0 (http://www.apache.org/licenses/LICENSE-2.0)  "
    print >>f, "                                                                 "
    print >>f, "***COMMANDs*** are:                                              "
    print >>f, "                                                                 "
    print >>f, "    * flyme help [NAME]"
    print >>f, "                                                                 "
    ...
    print >>f, "      Run the action separately."
    ...
    print >>f, "### Error Codes"
    print >>f, "                                                                 "
    print >>f, "You might confront with the following error while porting        "
    print >>f, "                                                                 "
    for code in sorted(help_model.CODE_TO_NAME.keys()):
        if code < 150:
            continue
